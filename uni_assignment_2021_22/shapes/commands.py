...
def remove_command(indexer: Indexer, shapes: list[Shape]):
    id = click.prompt("Please provide the id of the shape", type=click.IntRange(min=0, max_open=True))
...
def create_command(indexer: Indexer, shapes: list[Shape]):
    shape_type = click.prompt("Please provide shape type", type=click.Choice(SHAPES_MAP.keys()))
